...
module type FIRESTORE_OPS = sig
    ...
    module User : sig
        ...
        (** Retrieve the elo (if present) of an user and a certain game, return [Models.User.EloInfo.empty] if absent *)
        val get_elo : request:Dream.request -> user_id:string -> type_game:string -> Models.User.EloInfo.t Lwt.t
        (** Update the elo of the document for an user and a certain game *)
        val update_elo : request:Dream.request -> user_id:string -> type_game:string -> new_elo:Models.User.EloInfo.t -> unit Lwt.t
    end

    module Game : sig
        (** Retrieve a full game, from its id *)
        val get : Models.Game.t getter
        ...
        (** Create a game *)
        val create : request:Dream.request -> game:Models.Game.t -> string Lwt.t
        ...
        val add_event : request:Dream.request -> id:string -> event:Models.GameEvent.t -> unit Lwt.t

    end

    module ConfigRoom : sig
        (** Retrieve a config room as a JSON document, from its id *)
        val get : Models.ConfigRoom.t getter

        (** Create a config room, with a given id *)
        val create : request:Dream.request -> id:string -> config_room:Models.ConfigRoom.t -> unit Lwt.t

        (** Delete a config room *)
        val delete : deleter

        (** Add a candidate to the config room *)
        val add_candidate : request:Dream.request -> id:string -> candidate:Models.MinimalUser.t -> unit Lwt.t
...
module Make (FirestorePrimitives : FirestorePrimitives.FIRESTORE_PRIMITIVES) : FIRESTORE_OPS = struct
    ...
    module User = struct
        ...
        let update_elo = fun ~(request : Dream.request) ~(user_id : string) ~(type_game : string) ~(new_elo : Models.User.EloInfo.t) : unit Lwt.t ->
            let update: JSON.t = Models.User.EloInfo.to_yojson new_elo in
            FirestorePrimitives.update_doc ~request ~path:("users/" ^ user_id ^ "/elos/" ^ type_game) ~update

        let get_elo = fun ~(request : Dream.request) ~(user_id : string) ~(type_game : string) : Models.User.EloInfo.t Lwt.t ->
            let* doc : JSON.t Option.t = FirestorePrimitives.try_get_doc ~request ~path:("users/" ^ user_id ^ "/elos/" ^ type_game) in
            match doc with
            | None -> Lwt.return Models.User.EloInfo.empty
            | Some json ->
                begin match Models.User.EloInfo.of_yojson json with
                    | Error _ -> raise (Errors.UnexpectedError (Printf.sprintf "Invalid EloInfo for %s/%s: %s" user_id type_game (JSON.to_string json)))
    ...
    module Game = struct
        ...
        let get = fun ~(request : Dream.request) ~(id : string) : Models.Game.t Lwt.t ->
            get request ("parts/" ^ id) Models.Game.of_yojson
        ...
        let get_name = fun ~(request : Dream.request) ~(id : string) : string Lwt.t ->
            ...
            with JSON.Util.Type_error (_, _) ->
                raise (Errors.DocumentInvalid (Printf.sprintf "parts/%s" id))

        let create = fun ~(request : Dream.request) ~(game : Models.Game.t) : string Lwt.t ->
            let json : JSON.t = Models.Game.to_yojson game in
        ...
        let add_event = fun ~(request : Dream.request) ~(id : string) ~(event : Models.GameEvent.t) : unit Lwt.t ->
            let json = Models.GameEvent.to_yojson event in
    ...
    module ConfigRoom = struct
        ...
        let get = fun ~(request : Dream.request) ~(id : string) : Models.ConfigRoom.t Lwt.t ->
            get request ("config-room/" ^ id) Models.ConfigRoom.of_yojson

        let create = fun ~(request : Dream.request) ~(id : string) ~(config_room : Models.ConfigRoom.t) : unit Lwt.t ->
            let json = Models.ConfigRoom.to_yojson config_room in
        ...
        let add_candidate = fun ~(request : Dream.request) ~(id : string) ~(candidate : Models.MinimalUser.t) : unit Lwt.t ->
            let candidate_json = Models.MinimalUser.to_yojson candidate in
        ...
        let accept = fun ~(request : Dream.request) ~(id : string) : unit Lwt.t ->
            update ~request ~id ~update:(`Assoc [("partStatus", Models.ConfigRoom.GameStatus.(to_yojson Started))])
    end
