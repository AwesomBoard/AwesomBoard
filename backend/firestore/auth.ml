...
module Make
    ...
    let user_field : (string * Models.User.t) Dream.field =
        (* This is just a tag that we will store with a value alongside requests when handling them *)
        Dream.new_field ~name:"user" ()

    let get_user_field = fun (request : Dream.request) : (string * Models.User.t) ->
        match Dream.field request user_field with
        | None -> raise (Errors.UnexpectedError "No user stored. Is the Auth middleware missing?")
        | Some uid_and_user -> uid_and_user

    let get_user = fun (request : Dream.request) : Models.User.t ->
    ...
    let get_minimal_user = fun (request : Dream.request) : Models.MinimalUser.t ->
        let (uid, user) = get_user_field request in
        Models.User.to_minimal_user uid user
    ...
    let middleware = fun (project_id : string) : Dream.middleware ->
        fun (handler : Dream.handler) (request : Dream.request) ->
            let check_everything_and_process_request = fun () ->
                ...
                let uid : string =
                    match Jwt.verify_and_get_uid parsed_token project_id certificates with
                    | None -> raise (AuthError "Authorization token cannot be verified")
                    | Some uid -> uid in
                (* Get the user and check its verification status *)
                let* user : Models.User.t = FirestoreOps.User.get ~request ~id:uid in
                if user.verified then begin
                    ...
                    Stats.set_user request (Models.User.to_minimal_user uid user);
            ...
            | Errors.DocumentNotFound _ | Errors.DocumentInvalid _ ->
