...
module type CHAT = sig
    ...
    val add_message : Dream.request -> GameId.t -> Models.Message.t -> unit Lwt.t
    ...
    (** [iter_messages request game_id f] iterates over all messages of the chat
        ...
        [request]. *)
    val iter_messages : Dream.request -> GameId.t -> (Models.Message.t -> unit Lwt.t) -> unit Lwt.t
end
...
module ChatInMemory : CHAT = struct
    ...
    let add_message = fun (_request : Dream.request) (game_id : GameId.t) (message : Models.Message.t) : unit Lwt.t ->
        let old_messages = match Hashtbl.find_opt messages (GameId.to_int game_id) with
            | Some messages -> messages
            | None -> [] in
        let new_messages = message :: old_messages in
        Hashtbl.replace messages (GameId.to_int game_id) new_messages;
        Lwt.return ()

    let iter_messages = fun (_request : Dream.request) (game_id : GameId.t) (f : Models.Message.t -> unit Lwt.t) : unit Lwt.t ->
        let messages = match Hashtbl.find_opt messages (GameId.to_int game_id) with
            | Some messages -> messages
            | None -> [] in
...
module ChatSQL : CHAT = struct
    ...
    let add_message_query = t2 game_id message ->. unit @@ {|
        INSERT INTO messages (game_id, author_id, author_name, timestamp, content)
        VALUES (?, ?, ?, ?, ?)
    |}

    let add_message = fun (request : Dream.request) (game_id : GameId.t) (message : Models.Message.t) : unit Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
        Db.exec add_message_query (game_id, message)

    let get_messages_query = game_id ->* message @@ {|
        SELECT author_id, author_name, timestamp, content FROM messages
        WHERE game_id = ?
    |}

    let iter_messages = fun (request : Dream.request) (game_id : GameId.t) (f : Models.Message.t -> unit Lwt.t) : unit Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
