...
module type CONFIG_ROOM = sig
    ...
    val create : request:Dream.request -> ConfigRoom.t -> GameId.t Lwt.t

    (** [get ~request game_id] retrieves the config room for game [game_id]. If
        it does not exist, returns [None]. *)
    val get : request:Dream.request -> GameId.t -> ConfigRoom.t option Lwt.t

    val delete : request:Dream.request -> GameId.t -> unit Lwt.t
    ...
    (** [iter_active_rooms ~request f] retrieve the currently active games,
        ...
        [request] *)
    val iter_active_rooms : request:Dream.request -> (GameId.t -> ConfigRoom.t -> unit Lwt.t) -> unit Lwt.t
    ...
    val iter_candidates : request:Dream.request -> GameId.t -> (MinimalUser.t -> unit Lwt.t) -> unit Lwt.t
    ...
    (** [add_candidate ~request ~game_id candidate] makes [candidate] join the
        ...
        [request]. *)
    val add_candidate : request:Dream.request -> GameId.t -> MinimalUser.t -> unit Lwt.t

    (** [remove_candidate ~request ~game_id candidate] removes candidate
        [candidate] from the game [game_id], in the context of [request]. *)
    val remove_candidate : request:Dream.request -> GameId.t -> MinimalUser.t -> unit Lwt.t
    ...
    (** [select_opponent ~request ~game_id opponent] selects candidate
        ...
        [request]. *)
    val select_opponent : request:Dream.request -> GameId.t -> MinimalUser.t -> unit Lwt.t
...
module ConfigRoomSQL : CONFIG_ROOM = struct
    ...
    let create = fun ~(request : Dream.request) (config_room : ConfigRoom.t) : GameId.t Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
        Db.with_transaction (fun () ->
            match%lwt Db.exec create_query config_room with
            | Result.Ok () ->
                begin match%lwt Db.find get_last_inserted_id () with
                    | Result.Ok game_id -> Lwt.return (Result.Ok (GameId.GameId game_id))
                    | Result.Error e -> Lwt.return (Result.Error e)
                end
            | Result.Error e -> Lwt.return (Result.Error e))

    let get_query = game_id ->? config_room @@ {|
    ...
    let get = fun ~(request : Dream.request) (game_id : GameId.t) : ConfigRoom.t option Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
        Db.find_opt get_query game_id

    let delete_config_room_query = game_id ->. unit @@ {|
        DELETE FROM config_rooms
        WHERE id = ?
    |}

    let delete_candidates_query = game_id ->. unit @@ {|
        DELETE FROM candidates
        WHERE game_id = ?
    |}

    let delete = fun ~(request : Dream.request) (game_id : GameId.t) : unit Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
        Db.with_transaction (fun () ->
    ...
    let get_active_rooms_query = unit ->* t2 game_id config_room @@ {|
    ...
    let iter_active_rooms = fun ~(request : Dream.request) (f : GameId.t -> ConfigRoom.t -> unit Lwt.t) : unit Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
        Db.iter_s get_active_rooms_query (fun (game_id, config_room) ->
            Lwt.map Result.ok (f game_id config_room)) ()

    let get_candidates_query = game_id ->* MinimalUserSql.t @@ {|
        SELECT candidate_id, candidate_name
        FROM candidates
        WHERE id = ?
    |}

    let iter_candidates = fun ~(request : Dream.request) (game_id : GameId.t) (f : MinimalUser.t -> unit Lwt.t) : unit Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
        Db.iter_s get_candidates_query (fun candidate ->
            let+ result = f candidate in Result.Ok result) game_id

    let add_candidate_query = t3 game_id string string ->. unit @@ {|
        INSERT OR IGNORE INTO candidates (game_id, candidate_id, candidate_name)
        VALUES (?, ?, ?)
    |}

    let add_candidate = fun ~(request : Dream.request) (game_id : GameId.t) (candidate : MinimalUser.t) : unit Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
        Db.exec add_candidate_query (game_id, candidate.id, candidate.name)

    let remove_candidate_query = t2 game_id string ->. unit @@ {|
        DELETE FROM candidates
        WHERE id = ? AND candidate_id = ?
    |}

    let remove_candidate = fun ~(request : Dream.request) (game_id : GameId.t) (candidate : MinimalUser.t) : unit Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
        Db.exec remove_candidate_query (game_id, candidate.id)

    let select_opponent_query = t3 string string game_id ->. unit @@ {|
        UPDATE config_rooms
        SET chosen_opponent_id = ?, chosen_opponent_name = ?
        WHERE id = ?
    |}

    let select_opponent = fun ~(request : Dream.request) (game_id : GameId.t) (opponent : MinimalUser.t) : unit Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
        Db.exec select_opponent_query (opponent.id, opponent.name, game_id)

    let propose_config_query = t7 status game_type int int first_player string game_id ->. unit @@ {|
        UPDATE config_rooms
        SET status = ?, game_type = ?, move_duration = ?, game_duration = ?, first_player = ?, config = ?
        WHERE id = ?
    |}

    let propose_config = fun ~(request : Dream.request) (game_id : GameId.t) (proposal : ConfigRoom.Proposal.t) : unit Lwt.t ->
        Dream.sql request @@ fun (module Db : DB) -> check @@
    ...
    let change_status_query = t2 status game_id ->. unit @@ {|
        UPDATE config_rooms
        SET status = ?
        WHERE id = ?
    |}
    ...
    let change_status_to = fun (status : ConfigRoom.Status.t) ->
        fun ~(request : Dream.request) (game_id : GameId.t) : unit Lwt.t ->
            Dream.sql request @@ fun (module Db : DB) -> check @@
            begin
